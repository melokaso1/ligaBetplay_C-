...
namespace LigaBetplay.Modules.Estadisticas
{
    ...
    public class ServicioEstadisticas
    {
        ...
        public void MostrarEstadisticas()
        {
            ...
            Console.WriteLine(
                $"  Equipo mas goleador       : {masGoleador.Nombre} — {masGoleador.GF} goles a favor"
            );

            // Equipo menos goleador en contra (mejor defensa)
            var mejorDefensa = equipos.OrderBy(e => e.GC).First();
            Console.WriteLine(
                $"  Mejor defensa (menos GC)  : {mejorDefensa.Nombre} — {mejorDefensa.GC} goles en contra"
            );

            // Equipo mas ganador
            var masGanador = equipos.OrderByDescending(e => e.PG).First();
            Console.WriteLine(
                $"  Equipo mas ganador        : {masGanador.Nombre} — {masGanador.PG} victorias"
            );

            // Equipo mas perdedor
            var masPerdedor = equipos.OrderByDescending(e => e.PP).First();
            Console.WriteLine(
                $"  Equipo mas perdedor       : {masPerdedor.Nombre} — {masPerdedor.PP} derrotas"
            );
            ...
            Console.WriteLine(
                $"  Mejor constancia          : {mejorConstancia.Nombre} — {(double)mejorConstancia.PG / mejorConstancia.PJ:P0} de victorias"
            );
            ...
            Console.WriteLine(
                $"  Peor constancia           : {peorConstancia.Nombre} — {(double)peorConstancia.PP / peorConstancia.PJ:P0} de derrotas"
            );
            ...
            Console.WriteLine(
                $"  Equipos sin victoria      : {(sinVictoria.Any() ? string.Join(", ", sinVictoria.Select(e => e.Nombre)) : "Ninguno")}"
            );
            ...
            Console.WriteLine(
                $"  Fechas simuladas          : {_contexto.FechaActual - 1} de {totalFechas}"
            );
        }
    }
}
